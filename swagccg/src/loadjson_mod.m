function data = loadjson_mod(fname, varargin)
    % parse a JSON (JavaScript Object Notation) file or string, modified
    % to adjust and parse swagger API definition in a MatLab compatible
    % way. Plese note that this function and there subfunction are not
    % tested to work on non swagger files (i.e. not backwards compatible).
    % Modified by Erkan Demiralay 2018/12/06
    %
    % Call Signatures:
    ...
    %
    %         opt.SimplifyCell [0|1]: ``1``, loadjson will call cell2mat
    %                                 for each element of the JSON data, and group
    %                                 arrays based on the cell2mat rules.
    %
    %         opt.FastArrayParser [1|0|int]: ``1`` use a speed-optimized array parser when loading an
    %                                        array object. The fast array parser may
    %                                        collapse block arrays into a single large
    %                                        array similar to rules defined in cell2mat;
    %
    %                                        ``0`` to use a legacy parser;
    %
    %                                        if set to a larger-than-1 value,
    %                                        this option will specify the minimum
    %                                        dimension to enable the fast array parser.
    %                                        For example, if the input is a 3D array, setting
    %                                        FastArrayParser to 1 will return a 3D array;
    %
    %                                        ``2`` will return a cell array of 2D arrays;
    %
    %                                        ``3`` will return to a 2D cell array of 1D vectors;
    %
    %                                        ``4`` will return a 3D cell array.
    %
    ...
    global isoct 
    global arraytoken
    global current_object_depth %{{add}}
    current_object_depth = 0; %{{add}}
    global is_paths %{{add}}
    is_paths = false; %{{add}}
    global nth_path; %{{add}}
    nth_path = 1; %{{add}}
    ...
    if numel(string) == 0
        error('Please check path or URL. Could not read from:%s\n', fname);
    end 
    %{{/modify}}
    pos = 1; 
    len = length(string); 
    ...
    index_esc = 1;
    opt = struct(); %{{add}}
    %{{del}}opt = varargin2struct(varargin{:});
...
function object = parse_object(inStr, esc, varargin)
    global current_object_depth %{{add}}
    current_object_depth = current_object_depth + 1; %{{add}}
    global is_paths %{{add}}
    global nth_path %{{add}}
    ...
    if next_char(inStr) ~= '}'
        while 1
            ...
            if strcmp(str, 'paths') %{{add}}
                is_paths = true; %{{add}}
               % fprintf('paths was entered \n'); % debuging %{{add}}
            end %{{add}}
            ...
            if is_paths && current_object_depth == 2 %{{add}}
                alt_name = sprintf('path_%04d', nth_path); %{{add}}
                object.(alt_name).('methods') = val; %{{add}}
                object.(alt_name).('path') = str; %{{add}}
                nth_path =  nth_path + 1; %{{add}}
            else %{{add}}
    ...
    parse_char(inStr, '}');
    current_object_depth = current_object_depth - 1; %{{add}}
    if is_paths && current_object_depth == 1 %{{add}}
        is_paths = false; %{{add}}
        % fprintf('paths was was exited \n');  % debuging %{{add}}
    end %{{add}}
    if(isstruct(object))
...
function object = parse_array(inStr, esc, varargin)
	...
	if next_char(inStr) ~= ']'
        ...
        try
           ...
           %{{del}} if(regexp(arraystr, ':', 'once'))
           %{{del}}     error('One can not use MATLAB-like ":" construct inside a JSON array');
           %{{del}} end
        ...
        switch inStr(pos)
            ...
            case '\'
                if pos+1 > length(inStr)
                    msg = 'End of file reached right after escape character';
                    error_pos(msg, inStr);
                end
                pos = pos + 1;
                switch inStr(pos)
                    ...
                    case 'u'
                        if pos+4 > length(inStr)
                            msg = 'End of file reached in escaped unicode character';
                            error_pos(msg, inStr);
                        end
...
function num = parse_number(inStr, varargin)
    ...
    if(isoct ~= 0)
        token = '^\s*-?(?:0|[1-9]\d*)(?:\.\d+)?(?:[eE][+\-]?\d+)?';
        numstr = regexp(currstr, token, 'end');
